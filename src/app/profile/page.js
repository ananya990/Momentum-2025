...
function Profile() {
  ...
  useEffect(() => {
    // setLoading(true);
    // if (user) {
    //   setUsrDetails({tag: "MMT-123456o0"});
    //   setLoading(false);
    // }
  }, []);
      ...
      <div className="w-full md:h-full md:w-2/6 bg-[#0000005d] rounded-sm px-2 lg:px-7 py-3 md:py-8 flex flex-col items-center ">
        <div className="w-full flex gap-2 md:gap-4 items-center">
          ...
          <div className="flex flex-col h-full lg:space-y-1 md:py-4">
            ...
            <div className="text-sm lg:text-lg">{session?.user?.name || "Lakshay Yadav"}</div>
            <div className="text-xs font-thin">{session?.user?.email || "lionleo110@gmail.com"}</div>
          </div>
        </div>
